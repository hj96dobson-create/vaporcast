...
function AdminDashboard() {
  const [featureFlags, setFeatureFlags] = useState({
    promptEnhancer: true,
    storyboardGenerator: true,
    communityPublishing: true,
    announcementBanner: false,
  });
  const [announcementDraft, setAnnouncementDraft] = useState(
    "New creator workflow updates are rolling out this week.",
  );
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Control feature rollout and beta visibility.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {[
                { key: "promptEnhancer", label: "Prompt Enhancer" },
                { key: "storyboardGenerator", label: "Storyboard Generator" },
                { key: "communityPublishing", label: "Community Publishing" },
                { key: "announcementBanner", label: "Announcement Banner" },
              ].map((item) => (
                <div
                  key={item.key}
                ...
                >
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      setFeatureFlags((current) => ({ ...current, [item.key]: checked }))
                    }
                  />
                </div>
              ))}
          ...
          <Card>
            <CardHeader>
              <CardTitle>Roadmap Control</CardTitle>
              <CardDescription>Prioritize requests and moderation queue.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3 text-sm">
              <div className="rounded-xl border bg-muted/40 px-3 py-2">
                <p className="font-medium">Top request</p>
                <p className="text-muted-foreground">Auto B-roll scene generator</p>
              </div>
              <div className="rounded-xl border bg-muted/40 px-3 py-2">
                <p className="font-medium">Pending suggestions</p>
                <p className="text-muted-foreground">27 requires triage</p>
              </div>
              <div className="rounded-xl border bg-muted/40 px-3 py-2">
                <p className="font-medium">Moderation queue</p>
                <p className="text-muted-foreground">3 comments flagged for review</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Draft and publish product updates.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              <Textarea
                ...
                rows={5}
              />
              <button className="w-full rounded-xl bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground hover:opacity-90">
                Publish announcement
              </button>
              <p className="text-xs text-muted-foreground">
                Published notices appear in dashboard notifications.
              </p>
            </CardContent>
          </Card>
        </div>
