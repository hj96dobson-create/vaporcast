...
export async function handleWaitlistPost({ request }: { request: Request }) {
  let body: unknown;
  try {
  ...
  return Response.json({
    ok: true,
    message: "Check your inbox — we sent a confirmation link to secure your spot.",
    confirmedCount,
  });
}

export async function handleWaitlistConfirm({ request }: { request: Request }) {
...
export async function handleWaitlistTier() {
  const confirmedCount = await getConfirmedCount();
  const vipCap = 10;
